...
if __name__ == "__main__":
    ...
    params = []
    for k, a in zip(agents, env.agents):
        ...
        good_agent = False if a.adversary else True
        buffers[k] = Buffer(env, agents, args.n_steps, args.max_steps, args.gamma, args.gae, args.gae_lambda, device,
                            good_agent)

    j_critic = Critic(env, agents, args.h_size, args.n_hidden).to(device)
    j_c_optim = optim.Adam(list(j_critic.parameters()), lr=args.critic_lr, eps=1e-5)

    good_critic = Critic(env, [agents[-1]], args.h_size, args.n_hidden).to(device)
    good_c_optim = optim.Adam(list(good_critic.parameters()), lr=args.critic_lr, eps=1e-5)
    ...
    for update in range(1, n_updates + 1):
        if args.anneal_lr:
            ...
            j_c_optim.param_groups[0]["lr"] = frac * args.critic_lr
            good_c_optim.param_groups[0]["lr"] = frac * args.critic_lr
        ...
        j_c_h, good_c_h = [th.zeros((1, args.h_size)) for _ in range(2)]
        ...
        for step in range(args.n_steps):
            ...
            with th.no_grad():
                ...
                j_s_value, j_c_h = j_critic(j_observation, h=j_c_h)
                good_s_value, good_c_h = good_critic(observation[agents[-1]], h=good_c_h)
            ...
            for k, a in zip(agents, env.agents):
                buffers[k].store(
                    observation[k],
                    j_observation if a.adversary else observation[k],
                    action[k],
                    mean[k],
                    std[k],
                    logprob[k],
                    reward[k],
                    j_s_value if a.adversary else good_s_value,
                    done[k]
                )
        ...
        with th.no_grad():
            j_observation = _get_joint_obs(observation)
            j_value_, _ = j_critic(j_observation, j_c_h)
            good_value_, _ = good_critic(observation[agents[-1]], good_c_h)

            for k in agents:
                buffers[k].compute_mc(j_value_.reshape(-1) if k != agents[-1] else good_value_.reshape(-1))
        ...
        for k, a in zip(agents, env.agents):
            ...
            for epoch in range(args.n_epochs):
                ...
                if a.adversary:
                    values, _ = j_critic(b['j_observations'])
                    values = values.squeeze()

                    critic_loss = 0.5 * ((values - b['returns']) ** 2).mean()
                    critic_loss = critic_loss * args.v_coef

                    j_c_optim.zero_grad(True)
                    critic_loss.backward()
                    nn.utils.clip_grad_norm_(j_critic.parameters(), args.max_grad_norm)
                    j_c_optim.step()
                else:
                    values, _ = good_critic(b['j_observations'])
                    values = values.squeeze()

                    critic_loss = 0.5 * ((values - b['returns']) ** 2).mean()
                    critic_loss = critic_loss * args.v_coef

                    good_c_optim.zero_grad(True)
                    critic_loss.backward()
                    nn.utils.clip_grad_norm_(good_critic.parameters(), args.max_grad_norm)
                    good_c_optim.step()
