...
class Buffer:
    ...
    def __init__(self, env: gym.Env, agents: List[str], size: int, max_steps: int, gamma: float, gae: bool, gae_lambda: float, device: th.device, good_agent: bool = False):
        ...
        # self.b_obervations = th.zeros((self.size, self.o_space.shape[0] + self.a_space.shape[0])).to(device)
        j_obs_dim = self.o_space.shape[0] * len(agents)
        # j_obs_dim = (self.o_space.shape[0] + self.a_space.shape[0]) * len(agents)
        if good_agent: j_obs_dim = (self.o_space.shape[0] + self.a_space.shape[0]) #j_obs_dim = self.o_space.shape[0]
