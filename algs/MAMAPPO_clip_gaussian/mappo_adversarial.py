...
if __name__ == "__main__":
    ...
    for update in range(1, n_updates + 1):
        ...
        for k in agents:
            ...
            if args.wandb_log:
                wandb.log({f'Actor loss {k}': float(actor_loss),
                           f'Actor std {k} - Action 0': float(th.exp(actors[k].logstd[0])),
                           f'Actor std {k} - Action 1': float(th.exp(actors[k].logstd[1])),
                           f'Actor entropy {k}': float(entropy_loss)})

            for epoch in range(args.vf_epochs):
                values, _ = critics[k](b['j_observations'])
                # values = values.squeeze()[b['mask_j_observations']]
                values = values.reshape(b['mask_j_observations'].shape)[b['mask_j_observations']]
